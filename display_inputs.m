...
hold on;text(-0.5,0.2,'EX','Color','r');hold on;text(0.4,0.2,'IN','Color','b');
%EX and IN Vertical DIFFERENCE
subplot(2,3,2);hold on;
 for i=1:size(frac_diffv,1)
 exp=plot(frac_diffv(i,1:16)'*-1,1:16,'Color',[0.5 0.5 0.5]);set(gca,'Ydir','reverse');ylabel('Vertical input');xlabel('Fraction of total input');box off;
 exp.Color(4) = 0.2;
 end
...
mexp=errorbar(nanmean(frac_diffv(:,1:16))*-1,1:16,nanstd(frac_diffv(:,1:16))/sqrt(size(frac_diffv,1)),'horizontal','k');set(gca,'Ydir','reverse');
...
for i=1:size(frac_v,1)
exp=plot(frac_h(i,1:16)'*-1,'-r');xlabel('Horizontal input');ylabel('Fraction of total input');box off;
exp.Color(4) = 0.2;
exp=plot(frac_h(i,17:end)','-b');xlabel('Horizontal input');ylabel('Fraction of total input');box off;
exp.Color(4) = 0.2;
end
...
exp=plot(frac_diffh(i,1:16)*-1,'Color',[0.5 0.5 0.5]);xlabel('Horizontal input');ylabel('Fraction of total input');box off;
exp.Color(4) = 0.2;
end
...
;mexp=errorbar(nanmean(frac_diffh(:,1:16))*-1,nanstd(frac_diffh(:,1:16))/sqrt(size(frac_diffh,1)),'horizontal','k');
mexp.CapSize=3;
ylim([-0.1 0.1]);
