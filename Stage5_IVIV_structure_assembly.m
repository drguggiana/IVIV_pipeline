...
%% Keep only the cells with invivo data

% get the iviv vector
iviv_vector = ~cellfun(@isempty,{str.iviv})';
str = str(iviv_vector);
...
for cells = 1:cell_num
    % check if either the inhibition or the excitation maps are NaN, if so,
    % record it
...
%% OFF Fix the orientation fields to match the convention of the horizontal
% % bar being 0 degrees
% 
...
%     end
% end
%% OFF Calculate the maximum per layer subpixel
...
save(fullfile(stage5_fullIVIV_path,'str_iviv.mat'),'str')