...
for maps = 1:number_of_targets
    ...
    weighted_centroid = regionprops(target_maps(:,:,maps)~=0,...
        target_maps(:,:,maps),'Area','WeightedCentroid');
    ...
    centroid_vector(maps,:) = abs(corrected_centroid - target_somas(maps,:))/69;
end
