function [blanked_maps] = IMAPS_Control_interpolate(folder_all,ori_setup,aux_file_path,control_positions)
...
%% Load, trim and reorder the data for the interpolation 
...
%% Control interpolation of non-direct traces
...
% interp_cell = cell(control_positions,length(interp_var));

%for all the maps
for maps = 1:map_num
    
    fprintf(strjoin({'Current map:',num2str(stop_counter),'\r\n'},'_'))
    % if the stop counter is reached, stop
    if stop_counter == control_positions
        break
    end
    ...
    blanked_maps(stop_counter+1).real = curr_map(:,target_position);
    % blank the target position
    curr_map(:,target_position) = NaN;
    ...
    %get the 3d coordinates of the blanked responses
    blank_coord = find(isnan(curr_map));
    [qx,qy,qz] = ind2sub(size(curr_map),blank_coord);
    %and of the non-blank responses
    nonblank_coord = find(~isnan(curr_map));
    [dx,dy,dz] = ind2sub(size(curr_map),nonblank_coord);
    
    %for both kinds of interpolation
    for interpkind = 1:length(interp_var)
        filled_coord = griddatan([dx,dy,dz],curr_map(nonblank_coord),[qx,qy,qz],interp_var{interpkind});
        filled_map = curr_map;
        filled_map(blank_coord) = filled_coord;
%         %store the interpolated map
%         interp_cell{stop_counter,interpkind} = reshape(filled_map,[],num_positions);
        filled_map = reshape(filled_map,[],num_positions);
        blanked_maps(stop_counter+1).interp = filled_map(:,target_position);
    end
