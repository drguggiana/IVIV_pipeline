...
%for all cells 
if isempty(groups)==1
fig7= figure;set(fig7, 'Name', 'Input distribution');set(fig7, 'Position', [500, 150, 950, 150]);set(gcf,'color','w');
...
hold on;mexp=errorbar(nanmean(frac_diffv(:,2:16)),2:16,nanstd(frac_diffv(:,2:16))/sqrt(size(frac_diffv,1)),'horizontal','k');set(gca,'Ydir','reverse');
...
hold on;line([8.5 8.5], [-1 1],'Color','k','LineStyle','--');set(gca,'FontSize',8);
stats_g=[];
...
hold on;line([8.5 8.5], [-1 1],'Color','k','LineStyle','--');set(gca,'FontSize',8);
hold on;text(4,0.0575,'\Delta EX - IN zoom','Color','k');
