...
errorbar(nanmean(ind_tr1,2)/max(nanmean(ind_tr1,2)),nanstd(ind_tr1/max(nanmean(ind_tr1,2)),[],2)/sqrt(4),'-o','Color','k');
% plot(fit_ex1/max(fit_ex1),'k');
hold on;
text(140,0.25,['OSI= ' num2str(ori1)]);
hold on;box off
...
errorbar(nanmean(ind_tr2,2)/max(nanmean(ind_tr2,2)),nanstd(ind_tr2/max(nanmean(ind_tr2,2)),[],2)/sqrt(4),'-o','Color','b');
hold on; text(140,0.25,['OSI= ' num2str(ori2)],'Color','b');
ylabel('Normalized response');xlabel('Orientation (deg)');
