...
exp=plot(frac_v(i,2:16)',2:16,'-r');set(gca,'Ydir','reverse');ylabel('Vertical input');box off;
exp.Color(4) = 0.05;
exp=plot(frac_v(i,18:end)'*-1,2:16,'-b');set(gca,'Ydir','reverse');ylabel('Vertical map position');box off;
exp.Color(4) = 0.05;
end
hold on;
for i=1:size(frac_v,1)
exp=plot(frac_v(i,1:2)',1:2,'Color',[0.85 0.85 0.85],'LineStyle','-');set(gca,'Ydir','reverse');ylabel('Vertical input');box off;
exp.Color(4) = 0.05;
exp=plot(frac_v(i,17:18)'*-1,1:2,'Color',[0.85 0.85 0.85],'LineStyle','-');set(gca,'Ydir','reverse');ylabel('Vertical map position');box off;
exp.Color(4) = 0.05;
end
hold on;line([-1 1], [2.5 2.5],'Color','k','LineStyle','--');hold on;line([-1 1], [5.5 5.5],'Color','k','LineStyle','--');
hold on;line([-1 1], [7.5 7.5],'Color','k','LineStyle','--');hold on;line([-1 1], [11 11],'Color','k','LineStyle','--');
...
mexp.CapSize=3;xlim([-0.6 0.6]);xticks([-1:0.5:1]);hold on;ylim([1 16]);yticks([1:5:16]);yticklabels({'1','6','11','16'});xlabel('Fraction of total input')

hold on;mexp=errorbar(nanmean(frac_v(:,17:18))*-1,1:2,nanstd(frac_v(:,17:18))/sqrt(size(frac_v,1)),'horizontal','Color',[0.5 0.5 0.5]);set(gca,'Ydir','reverse');
hold on;text(-0.55,4.5,'L2/3');hold on;text(-0.55,7,'L4');hold on;text(-0.55,9.5,'L5');set(gca,'FontSize',10);
hold on;text(-0.5,0.2,'IN','Color','b');hold on;text(0.4,0.2,'EX','Color','r');
%EX and IN Vertical DIFFERENCE
hold on;
%  for i=1:size(frac_diffv,1)
% 
%  exp=plot(frac_diffv(i,2:16)',2:16,'Color',[0.5 0.5 0.5]);set(gca,'Ydir','reverse');xlabel('\Delta Fraction of total input');box off;
%  exp.Color(4) = 0.2;
%  end
%  hold on;
%   for i=1:size(frac_diffv,1)
%  exp=plot(frac_diffv(i,1:2)',1:2,'Color',[0.85 0.85 0.85]);set(gca,'Ydir','reverse');xlabel('\Delta Fraction of total input');box off;
%  exp.Color(4) = 0.2;
%  end
hold on;mexp=errorbar(nanmean(frac_diffv(:,2:16)),2:16,nanstd(frac_diffv(:,2:16))/sqrt(size(frac_diffv,1)),'horizontal','m');set(gca,'Ydir','reverse');
mexp.CapSize=3;xlim([-0.6 0.6]);
hold on;mexp=errorbar(nanmean(frac_diffv(:,1:2)),1:2,nanstd(frac_diffv(:,1:2))/sqrt(size(frac_diffv,1)),'horizontal','Color',[0.5 0.5 0.5]);set(gca,'Ydir','reverse');
mexp.CapSize=3
xticks([-0.6:0.3:0.6]);hold on;text(-0.05,0.2,'\Delta','Color','m');
% hold on;ylim([1 16]);yticks([1:5:16]);yticklabels({'1','6','11','16'});
% 
% hold on;line([-1 1], [2.5 2.5],'Color','k','LineStyle','--');hold on;line([-1 1], [5.5 5.5],'Color','k','LineStyle','--');
% hold on;line([-1 1], [7.5 7.5],'Color','k','LineStyle','--');hold on;line([-1 1], [11 11],'Color','k','LineStyle','--');
% set(gca,'FontSize',8);text(-0.3,0.2,'\Delta EX - IN','Color','k');
